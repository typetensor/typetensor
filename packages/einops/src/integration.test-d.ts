...

{
  ...
  type Valid =
    Pattern extends TypeParseError<string>
      ? false
      : ValidatePattern<Pattern['input'], Pattern['output']>;
...

{
  ...
  expectTypeOf<Valid>().toEqualTypeOf<
    ValidationResult<false, 'Output validation failed: Duplicate axis names in output'>
  >();
}
...

{
  ...
  expectTypeOf<Valid>().toEqualTypeOf<
    ValidationResult<false, 'Input validation failed: Empty composite patterns not allowed'>
  >();
}
