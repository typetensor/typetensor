...
export function compareWithHistory(
  ...
  try {
    ...
    for (const [scenarioName, currentMetrics] of Object.entries(current.scenarios)) {
      ...
      const opsChange =
        ((currentMetrics.ops_per_sec - prevMetrics.ops_per_sec) / prevMetrics.ops_per_sec) * 100;
      const latencyChange =
        ((currentMetrics.mean_ns - prevMetrics.mean_ns) / prevMetrics.mean_ns) * 100;
      ...
      console.log(
        `  Throughput: ${opsDirection} ${opsChange.toFixed(2)}% (${currentMetrics.ops_per_sec.toFixed(0)} ops/sec)`,
      );
      console.log(
        `  Latency: ${latencyDirection} ${latencyChange.toFixed(2)}% (${formatLatency(currentMetrics.mean_ns)})`,
      );

      // Flag potential regressions
      if (opsChange < -5) {
        console.log(
          `  ⚠️  POTENTIAL REGRESSION: Throughput dropped by ${Math.abs(opsChange).toFixed(2)}%`,
        );
      }
      if (latencyChange > 10) {
        console.log(
          `  ⚠️  POTENTIAL REGRESSION: Latency increased by ${latencyChange.toFixed(2)}%`,
        );
      }
      console.log('');
    }
