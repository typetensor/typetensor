...
async function runBackendComparison() {
  ...
  for (const testConfig of testConfigs) {
    ...
    if (cpuResult && wasmResult) {
      const comparison: BackendComparison = {
        ...
        winner:
          wasmResult.hz > cpuResult.hz * 1.05
            ? 'WASM'
            : cpuResult.hz > wasmResult.hz * 1.05
              ? 'CPU'
              : 'Tie',
      };
      results.push(comparison);
      console.log(
        `   ${comparison.operation}: ${comparison.winner} wins (${comparison.speedup.toFixed(2)}x)`,
      );
    }
  }
  ...
  for (const testConfig of testConfigs.slice(0, 2)) {
    // Skip large for unary to save time
    const data = generateRandomData(testConfig.shape);
  ...
  for (const testConfig of testConfigs.slice(0, 2)) {
    // Skip large for binary to save time
  ...
  const categories = {
    ...
    Unary: results.filter((r) =>
      ['neg', 'abs', 'sin', 'exp', 'sqrt'].some((op) => r.operation.includes(op)),
    ),
  ...
  for (const [category, categoryResults] of Object.entries(categories)) {
    ...
    const categoryAvgSpeedup =
      categoryResults.reduce((sum, r) => sum + r.speedup, 0) / categoryResults.length;

    console.log(
      `   ${category}: ${categoryWasmWins}/${categoryResults.length} WASM wins, ${categoryAvgSpeedup.toFixed(2)}x avg speedup`,
    );
  }
