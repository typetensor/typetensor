...
describe('EinopsScanner', () => {
  ...
  describe('Realistic Patterns', () => {
    it('should handle transpose pattern', () => {
      ...
      expect(tokenTypes).toEqual([
        'axis', 'whitespace', 'axis', 'whitespace', 
        'arrow', 'whitespace', 'axis', 'whitespace', 'axis'
      ]);
  ...
  describe('Composite Patterns', () => {
    it('should tokenize simple composite pattern', () => {
      ...
      const tokenTypes = result.tokens.map(t => t.type);
      expect(tokenTypes).toEqual([
        'lparen', 'axis', 'whitespace', 'axis', 'rparen', 'whitespace', 'axis'
      ]);
    ...
    it('should handle composite transformation', () => {
      ...
      const tokenTypes = result.tokens.map(t => t.type);
      expect(tokenTypes).toEqual([
        'lparen', 'axis', 'whitespace', 'axis', 'rparen', 'whitespace', 
        'axis', 'whitespace', 'arrow', 'whitespace', 
        'axis', 'whitespace', 'axis', 'whitespace', 'axis'
      ]);
    });
    ...
    it('should handle nested composites', () => {
      ...
      const tokenTypes = result.tokens.map(t => t.type);
      expect(tokenTypes).toEqual([
        'lparen', 'lparen', 'axis', 'whitespace', 'axis', 'rparen', 'whitespace', 'axis', 'rparen'
      ]);
    });
...
describe('Scanner Error Handling', () => {
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle very long axis names', () => {
      const longName = 'very_long_axis_name_that_goes_on_and_on_with_many_underscores_and_numbers_123';
