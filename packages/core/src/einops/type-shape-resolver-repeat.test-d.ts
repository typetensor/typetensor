...
// Test 3: Mixed repetition and new axes
{
  ...
  type Result3 = ValidRepeatPattern<'h w -> (h h2) (w w2) c', readonly [16, 16], { h2: 2, w2: 2, c: 3 }>;
  ...
  // Interleaved pattern
  type Result5 = ValidRepeatPattern<'h w -> batch (h h2) c (w w2)', readonly [2, 3], { batch: 4, h2: 2, c: 3, w2: 2 }>;
...
// Test 4: Complex composite patterns
{
  ...
  type Result1 = ValidRepeatPattern<'(h h2) w -> h (w w3)', readonly [4, 6], { h: 2, h2: 2, w3: 3 }>;
  expectTypeOf<Result1>().toEqualTypeOf<readonly [2, 18]>();

  // Nested composite with new axes
  type Result2 = ValidRepeatPattern<'(h (h2 h3)) w -> h h2 w c', readonly [8, 6], { h: 2, h2: 2, h3: 2, c: 3 }>;
  expectTypeOf<Result2>().toEqualTypeOf<readonly [2, 2, 6, 3]>();

  // Multiple composite patterns
  type Result3 = ValidRepeatPattern<'(a a2) (b b2) -> (a a3) b c', readonly [4, 6], { a: 2, a2: 2, b: 3, b2: 2, a3: 3, c: 4 }>;
  ...
  type Result5 = ValidRepeatPattern<'(a (b b2)) -> a b c d', readonly [8], { a: 2, b: 2, b2: 2, c: 3, d: 4 }>;
...
// Test 5: Ellipsis with new axes
{
  ...
  // Ellipsis in middle
  type Result4 = ValidRepeatPattern<'batch ... c -> batch ... c d', readonly [4, 32, 32, 3], { d: 2 }>;
...
// Test 6: Ellipsis with repetition
{
  ...
  expectTypeOf<Result1>().toEqualTypeOf<readonly [8, 8]>();
  ...
  expectTypeOf<Result5>().toEqualTypeOf<readonly [4, 16, 16]>();
}
...
// Test 9: Singleton dimensions
{
  ...
  // Multiple singletons
  type Result3 = ValidRepeatPattern<'1 h 1 w 1 -> a h b w c', readonly [1, 2, 1, 3, 1], { a: 2, b: 4, c: 5 }>;
...
// Test 10: Computer vision patterns  
{
  ...
  type Result4 = ValidRepeatPattern<'h w -> (h h2) (w w2) c', readonly [16, 16], { h2: 2, w2: 2, c: 3 }>;
...
// Test 11: Time series patterns
{
  ...
  type Result3 = ValidRepeatPattern<'time -> (time t2) features', readonly [25], { t2: 4, features: 128 }>;
  expectTypeOf<Result3>().toEqualTypeOf<readonly [100, 128]>();

  // Sequence batching
  type Result4 = ValidRepeatPattern<'time features -> batch time features', readonly [50, 64], { batch: 16 }>;
  expectTypeOf<Result4>().toEqualTypeOf<readonly [16, 50, 64]>();

  // Time series expansion
  type Result5 = ValidRepeatPattern<'... -> batch ... features', readonly [100], { batch: 8, features: 32 }>;
...
// Test 16: Rank mismatch
{
  ...
  expectTypeOf<Result3>().toEqualTypeOf<"[Repeat ❌] Shape Error: Repeat pattern expects 1 dimensions but tensor has 2">();
...
{
  type Result1 = ValidRepeatPattern<'(h h2) w -> h w c', readonly [4, 6], false, { h: 3, h2: 2, c: 3 }>;
  ...
  type Result2 = ValidRepeatPattern<'h (w w2) -> h w c', readonly [2, 9], false, { w: 2, w2: 3, c: 4 }>;
  ...
  type Result3 = ValidRepeatPattern<'(a a2) (b b2) -> a b c', readonly [6, 8], false, { a: 2, a2: 2, b: 3, b2: 2, c: 4 }>;
  ...
  type Result4 = ValidRepeatPattern<'((a a2) a3) -> a c', readonly [12], false, { a: 2, a2: 2, a3: 2, c: 3 }>;
  ...
  type Result5 = ValidRepeatPattern<'(h h2) -> (h h2) c', readonly [5], false, { h: 2, h2: 3, c: 4 }>;
...
// Test 18: Valid patterns should return shapes, not error messages
{
  ...
  type Result7 = ValidRepeatPattern<'h w -> (h h2) (w w2) c d', readonly [2, 3], { h2: 2, w2: 3, c: 4, d: 5 }>;
  expectTypeOf<Result7>().toEqualTypeOf<readonly [4, 9, 4, 5]>();

  // Multiple ellipsis dims
  type Result8 = ValidRepeatPattern<'... -> batch ... features', readonly [32, 32, 3], { batch: 8, features: 64 }>;
  expectTypeOf<Result8>().toEqualTypeOf<readonly [8, 32, 32, 3, 64]>();

  // Nested composite
  type Result9 = ValidRepeatPattern<'(h (h2 h3)) -> h h2 c', readonly [8], { h: 2, h2: 2, h3: 2, c: 4 }>;
